...
$devs = Get-CMDeviceCollectionDirectMembershipRule -CollectionName "List upgraded to 10 Forced"

foreach ($dev in $devs) {
   $devinfo = "" | select "name", "os"
  # $gdev = Get-CMDevice -Name $dev.RuleNAme
   $os = Get-OSinfoFromSCCM -ComputerName $dev.RuleName -SCCMServer sccm.kontur
   Write-Host "--: " $os.ComputerName " - " $os.OS
   $devinfo.name = $os.ComputerName
   $devinfo.os = $os.OS
   $all0 += $devinfo
   $devinfo = $null
